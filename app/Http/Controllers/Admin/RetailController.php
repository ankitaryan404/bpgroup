...
class RetailController extends Controller
{
    ...
    {
        $data = retail::all();
        return view('admin/retail/property_list', ['data' => $data]);
    }
    public function manage(Request $req, $id = '')
    {
        if ($id > 0) {
            $arr = retail::where('id', $id)->get();
            ...
            $result['lease_term'] = $arr['0']->lease_term;
            $result['cc'] = $arr['0']->cc;
            $result['fire_noc'] = $arr['0']->fire_noc;
            $result['floor_height'] = $arr['0']->floor_height;
            $result['comp_occ_cert'] = $arr['0']->comp_occ_cert;
            $result['power_back'] = $arr['0']->power_back;
            $result['power_load'] = $arr['0']->power_load;
            $result['stamp_duty'] = $arr['0']->stamp_duty;
            $result['rent_free_period'] = $arr['0']->rent_free_period;
            ...
            $result['fire_noc'] = '';;
            $result['floor_height'] = '';
            $result['comp_occ_cert'] = '';
            $result['power_back'] = '';
            $result['stamp_duty'] = '';
            $result['rent_free_period'] = '';
            $result['notice_period'] = '';
            $result['id'] =0;
            $result['status'] = '';
        }
        return view('admin/retail/add_property', $result);
    }
    public function manage_retail(Request $req)
    {
        $req->validate([
            'slug' => 'required|unique:retails,slug,' . $req->post('id'),
        ]);


        if ($req->id > 0) {
            $data = retail::find($req->id);
        ...
        if ($req->hasfile('banner_pic')) {
            if ($req->post('id') > 0) {
                $arrImage = DB::table('retails')->where(['id' => $req->post('id')])->get();
    ...
    public function detail($id)
    {
        ...
        return view('admin/retail/detail', ['data' => $data, 'data2' => $data2]);
    }
    public function search($str)
    {
        $data = DB::table('retais')
