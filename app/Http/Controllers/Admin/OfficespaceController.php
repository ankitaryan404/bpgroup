...
class OfficespaceController extends Controller
{
    ...
    {
        $data = officespace::all();
        return view('admin/officespace/property_list', ['data' => $data]);
    }
    public function manage(Request $req, $id = '')
    {
        if ($id > 0) {
            $arr = officespace::where('id', $id)->get();
            ...
            $result['ideal_for_business'] = $arr['0']->ideal_for_business;
            ...
            $result['personal_washroom'] = $arr['0']->personal_washroom;
            ...
            $result['pantry'] = '';
            $result['no_toilet'] = '';
            $result['personal_washroom'] = '';
            $result['floor_plan'] = '';
           
            $result['id'] = 0;
            $result['status'] = '';
        }
        return view('admin/officespace/add_property', $result);
    }
    public function manage_officespace(Request $req)
    {
        ...
        return redirect('admin/officespace');
    }
    public function status(Request $req, $status, $id)
    {
        $data = officespace::find($id);
        $data->status = $status;
        $data->save();
    ...
    public function detail($id)
    {
        ...
        return view('admin/officespace/detail', ['data' => $data, 'data2' => $data2]);
    }
    public function search($str)
    {
        $data = DB::table('officespaces')
